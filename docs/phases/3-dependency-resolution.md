...
- [x] Cache expiry (24 h TTL) and manual refresh (`rosup resolve --refresh`)
- [x] Store cache YAML in `~/.rosup/cache/{distro}-cache.yaml`
...
- [x] Support user-defined overrides from `[resolve.overrides]` in `rosup.toml`
...
- [x] Run resolution as a pre-step for `rosup build` and `rosup test`
...
- [x] Replace `RoxDir` with `GlobalStore` and `ProjectStore`:
  - `GlobalStore { cache: PathBuf, src: PathBuf }` — `~/.rosup/cache/` and `~/.rosup/src/`
  - `ProjectStore { src: PathBuf, build: PathBuf, install: PathBuf }` — `<root>/.rosup/`
- [x] `GlobalStore::open()` — resolves from `~/.rosup/`
- [x] `ProjectStore::open(root)` — resolves from `<root>/.rosup/`
...
- [x] Rewrite `source_pull` to use bare clone + git worktree:
  1. Bare clone keyed by repo name into `~/.rosup/src/<repo>.git`
     (`git clone --bare <url> <bare>`, or `git -C <bare> fetch --quiet origin`)
  2. Worktree per project: `git -C <bare> worktree add --detach <project>/.rosup/src/<repo> <target>`
...
- [x] `rosup init` adds `.rosup/` to the project `.gitignore`

## Acceptance Criteria

- [x] `rosup resolve --dry-run` lists each dep with its resolution method
...
- [x] `[resolve.overrides.X]` in `rosup.toml` overrides the source URL for dep X
- [x] Resolution runs automatically before `rosup build` unless `--no-resolve` is passed
- [x] `rosup resolve` with no args resolves all deps for all workspace members
- [x] Errors are clear when a dep is not found in any tier
- [x] `~/.rosup/src/<repo>.git` is a bare clone; per-project `.rosup/src/<repo>/` is a worktree
...
- [x] `.rosup/` is added to `.gitignore` by `rosup init`
