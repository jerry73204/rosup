...
After all packages are built, generate:

### Option A: Ship `_local_setup_util_sh.py`

Copy colcon's Python helper unchanged. Guarantees compatibility.

### Option B: Generate static shell scripts from Rust

Since we have the topological order and all DSV files, generate
`local_setup.bash` that sources packages in the correct order without
a Python helper:

```bash
# Generated by rosup — no Python needed at source-time
_rosup_source() { [ -f "$1" ] && . "$1"; }
COLCON_CURRENT_PREFIX="<install>/pkg_a" _rosup_source "<install>/pkg_a/share/pkg_a/package.sh"
COLCON_CURRENT_PREFIX="<install>/pkg_b" _rosup_source "<install>/pkg_b/share/pkg_b/package.sh"
```

This eliminates the Python dependency at source-time.

**Decision: Option B by default, with `_local_setup_util_sh.py` also
copied for tools that expect it (e.g. `colcon test-result`).**
